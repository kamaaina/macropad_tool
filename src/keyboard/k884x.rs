...
impl Keyboard for Keyboard884x {
    fn program(&mut self, macropad: &Macropad) -> Result<()> {
        ...
        for (i, layer) in macropad.layers.iter().enumerate() {
            ...
            if self.pid == 0x8842 {
                // seems to have an extra message
                let msg = [
                    0x3, 0xfb, 0xfb, 0xfb, 0x3d, 0x6, 0x8c, 0xd0, 0x80, 0x0, 0x3c, 0xd0, 0x80, 0x0,
                    0xcc, 0xd2, 0x19, 0x1, 0x40, 0xd0, 0x80, 0x0, 0x8c, 0xd0, 0x80, 0x0, 0x0, 0x0,
                    0x0, 0x0, 0x10, 0x3c, 0x48, 0x0, 0x5c, 0xd0, 0x80, 0x0, 0x78, 0x10, 0x7, 0x7,
                    0x98, 0xd0, 0x80, 0x0, 0x70, 0xf5, 0x1e, 0x62, 0x98, 0xda, 0x11, 0x62, 0xc,
                    0x80, 0x0, 0x0, 0x18, 0x83, 0x3c, 0x6, 0xff, 0xff, 0xff,
                ];
                self.send(&msg)?;
            }
        }
